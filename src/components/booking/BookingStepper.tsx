...
import { BOOKING_STEP_LABELS, type BookingStep } from '@/types/booking-flow';
...
export function BookingStepper({ currentStep, completedSteps, className }: BookingStepperProps) {
  const steps: BookingStep[] = [1, 2, 3, 4];

  const isStepCompleted = (step: BookingStep) => completedSteps.includes(step);
  const isStepActive = (step: BookingStep) => step === currentStep;
  const isStepInactive = (step: BookingStep) => step > currentStep && !isStepCompleted(step);
    ...
    >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-neutral-900">
                {BOOKING_STEP_LABELS[currentStep]}
              </p>
            </div>
          </div>
      ...
      <div className="sr-only" aria-live="polite" aria-atomic="true">
        {isStepActive(currentStep) &&
          `Now on step ${currentStep}: ${BOOKING_STEP_LABELS[currentStep]}`}
      </div>
    </div>
  );
}
